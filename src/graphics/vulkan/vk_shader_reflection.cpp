...
static std::map<Descriptor2::Type, std::string> type_name
{
...
static std::map<VkShaderStageFlagBits, std::string> stage_name
{
...
ShaderReflectionData GetShaderReflection(const std::string &name, Shader::Type type, const uint32_t *spirv, size_t word_count)
{
	...
	for (auto &resource: resources.sampled_images)
	{
		...
		if (!spirv_type.array.empty())
		{
			//Log() << "size" << spirv_type.array.size() << "back" << spirv_type.array.back() << "[0]" << spirv_type.array[0];
			array_size = spirv_type.array[0];
		}
...
std::vector<uint32_t> CompileShader(const std::string &name, Shader::Type type, const std::string &source)
{
	...
	switch (type)
	{
		case Shader::Type::VERTEX: stage = shaderc_glsl_vertex_shader; break;
		case Shader::Type::FRAGMENT: stage = shaderc_glsl_fragment_shader; break;
		case Shader::Type::GEOMETRY: stage = shaderc_glsl_geometry_shader; break;
		case Shader::Type::COMPUTE: stage = shaderc_glsl_compute_shader; break;
	}
									  ...
									  VkPipelineLayoutCreateFlags flags)
{
	...
	for (auto &set: combined_descriptors)
	{
		...
		for (uint32_t i = 0; i < set.size(); i++)
			flags[i] = (bindings.at(i).descriptorCount > 1) ?
